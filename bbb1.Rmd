...
Jedes R Paket ist zu finden auf [link](http://cran.r-project.org). Herunterladen können wir es über die Command line durch: `install.packages("ggplot2")`.   

### library

Mit dem Befehl `library("ggplot2")` können wir es zugänglich machen. Ersetze ggplot2 natürlich beliebig. 

`qplot` erstellt schnelle Plots. Zwei Vektoren der selben Länge und schon haben wir einen Scatterplot erstellt. Hierbei wird ein Vektor mithilfe der `c` Funktion erstellt. In den Klammern getrennt durch Komma stehen die einzelnen Werte.
...
Wir rufen also `?sample` auf. Hier stehen viele wichtige Infos, die wir zum bauen unserer Funktion brauchen. `sample(x, size, replace = FALSE, prob = NULL)`.  
...
```{r}
gender <- factor(c("male", "femle", "female", "male"))
typeof(gender)
attributes(gender)
unclass(gender)
gender
...
*Lösung*  Diese Übung geht nicht klar. Jeder Vektor kann nur einen Datentypen annehmen. R zwingt all deine Werte zu einem Character String. 
...
Ist ein Character String in einem Vector vorhanden, so zwingt R alle anderen Elemente ebenfalls in einen Character String. Sind Zahlen und boolsche Werte (T und F) vorhanden, so konvertiert R diese zu Zahlen. `TRUE` wird zu 1 und `FALSE` wird zu 0.  
...
Die Struktur von Listen kann sehr kompliziert und lang werden. Aber die Flexibilität macht Listen zu einem sehr nützlichen Ablageort für verschiedenste Daten und Informaionen. Mit Listen kannst du alles zusammen lagern.  
...
Ein ganzes Deck Karten lannst du auch so speichern. Als Liste mit Unter-/Sublisten. Du kannst aber einfach eine spezielle Klasse von Listen benutzen. Einen Data Frame.
...
R hat die Character Strings als Faktoren abgespeichert. Du kannst dies verhindern durch das Argument `stringsAsFactors = FALSE`.
...
Vergesse nicht die Gänsefüßchen. Sonst sucht R ein Objekt namens `ace`. Du kannst diese Methode verwenden, um Asse zu finden und dann in deinem Deck zu ersetzen. Sogar nachdem gemischt wurde. Bilde erst einen logicl test, um die Asse zu identifizieren. Dann benutze den logical Vektor als Index.
...
Gebe jedem Herz einen Wert von `1`. 
...
In Hearts hat die Pik Dame (queen spades) hat eine/n Kartenzahl/value von 13. Schauen wir uns die Damen an und die Piks.
...
Operator      | Syntax            | Test
-----------   | ----------        | ---------------
`&`           | `cond1 & cond2`           | Sind sowohl cond1 als auch cond2 wahr?
`|`           | `cond1 | cond2`          | Ist mind. eins von cond1, cond2 wahr?
`xor`         | `xor(cond1, cond2)`           | Ist genau eins von cond1, cond2 wahr?
`!`           | `!cond1`                | Ist cond1 falsch? Umkehrung von logical test
`any`         | `any/cond1, cond2, ...)`          | Sind irgendwelche der Conditions wahr?
`all`         | `all(cond1, cond2, ...)`          | Sind alle der Conditions wahr? gleich b?
...
Die meisten R Funktionen bieten ein optionales Arguemnt an, `na.rm`, das für `NA` remove steht. R ignoriert die `NA`s, wenn du das Argument `na.rm = TRUE` hinzufügst.
...
R verknüpft die runtime environmet einer Funktion zu der environment, in der die Funktion zuerst kreiert wurde.
...
Jetzt reinigt `deal` die gloale Kopie von `deck`.
...
In diesem Kapitel wollen wir eine slot machine aufbauen. Die `play` Funktion soll zwei Dinge tun. 
...
Die entsprechenden Wahrscheinlichkeiten sind gegeben. Die Manitoba slot machine benutzt ein kompliziertes Auszahlungsschema. Man gewinnt bei:  
...
Um unsere slot machine zu programmieren weren wir einen komplexen Algorithmus anwenden. Benutze dazu eine simple Strategie.
...
Ein anderer Weg eine Aufgabe aufzuteilen ist es Gruppen ähnlicher Fälle innerhalb der Aufgabe zusammenzufassen.  Haben wir bei unserer slot machine drei gleiche, so ist ein anderer Algorithmus anzuwenden, als wenn wir drei verschiedene Symbole haben. `score` benutzt nie alle Algorithmen auf einmal. Immer nur einen, abhängig von der Kombination der Symbole. 
...
Kümmere dich also nur um eine Unteraufgabe auf einmal. Bau dir ein konkretes Beispiel und test alle Unteraufgaben separat. 
...
Wenn du `print` auf einen numrischen Vektor ausführst, eine Zahl wird ausgegeben. 
...
Wenn du Objekte mit Klassen benutzt, so findest du dich im S3 System wider. Dieses besteht aus:  
...
`play` gibt jetzt sowohl den Preis, als auch die angehängten Symbole mit dem Preis wider. Preis und Symbole bleiben als Paar zusammen, auch wenn wir sie an ein neues Objekt übergeben.
...
Jetzt wollen wir sichergehen, dass jeder Output die Klasse `slot` hat.
...
Als Ergebnis gibt R alles in unserem schönen Format wider.
...
Schauen wir uns unsere slot machine und die `score` Funktion an. Ihr wird Betrug vorgeworfen! Das lässt sich natürlich nur überprüfen, wenn wir das Programm einige Male laufen lassen. Einige sehr viele Male natürlich.
...
Lass uns jetzt den Erwartungswert für die slot machine berechnen. 
...
Für jedes Mitglied der Menge `that` wird der Code zwischen den geschweiften Klammern ausgeführt. Im folgenden Beispiel wird der `print` Befehl für jedes Element der Menge (Caracter Strings) ausgeführt. Also hier genau 4 mal.
...
Anstatt auf eine Menge von Objekten deine Schleife auszuführen, führe sie auf ganzen Zahlen aus, und indizere auf dein Objekt und deinen Speicher-Vektor.
...
Statt der `if` Baum und `for` Kombination, könntest du verktorisierten Code benutzen. Es werden viele Kombinationen ausgegeben, auf die du dann mit logical Subsetting zugreifen kannst. Auf alle gleichzeitig.  
